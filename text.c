...
void
g15r_renderCharacterLarge (g15canvas * canvas, int col, int row,unsigned char character, unsigned int sx,unsigned int sy){
...
void draw_ttf_str (g15canvas * canvas, char *str, int x, int y, int color,FT_Face face){
	...
	for (i = 0; i < len; i++){
		errcode =
		FT_Load_Char (face, str[i],FT_LOAD_RENDER | FT_LOAD_MONOCHROME | FT_LOAD_TARGET_MONO);
...
void g15r_ttfPrint (g15canvas * canvas, int x, int y, int fontsize, int face_num, int color, int center, char *print_string){
	...
	if (canvas->ttf_fontsize[face_num]){
		if (fontsize > 0 && FT_IS_SCALABLE (canvas->ttf_face[face_num][0])){
			canvas->ttf_fontsize[face_num] = fontsize;
			int errcode =
			FT_Set_Pixel_Sizes (canvas->ttf_face[face_num][0], 0,canvas->ttf_fontsize[face_num]);
			if (errcode)
				printf ("Trouble setting the Glyph size!\n");
		}
		y =
		calc_ttf_true_ypos (canvas->ttf_face[face_num][0], y,canvas->ttf_fontsize[face_num]);
		if (center == 1)
